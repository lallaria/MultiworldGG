        ...
        if self.connected:
            try:
                ...
                msg = (f"Error reading game memory! (Did the game crash?)\n"
                       ...
                       f"from the MultiworldGG Launcher.\n"
                       ...
                       f"   Then close and reopen the Jak and Daxter Client from the MultiworldGG Launcher.")
    ...
    async def connect(self):
        ...
        if marker_address:
            ...
            self.goal_address = int.from_bytes(self.gk_process.read_bytes(goal_pointer, sizeof_uint64),
                                               byteorder="little",
                                               signed=False)
            logger.debug("Found the MultiworldGG memory address: " + str(self.goal_address))
            await self.verify_memory_version()
        else:
            self.log_error(logger, "Could not find the MultiworldGG marker address!")
            self.connected = False

    async def verify_memory_version(self):
        if self.goal_address is None:
            self.log_error(logger, "Could not find the MultiworldGG memory address!")
            self.connected = False
            return
        ...
        try:
            ...
            if memory_version is None:
                ...
                msg = (f"The OpenGOAL memory structure is incompatible with the current MultiworldGG client!\n"
                       f"   Expected Version: {str(expected_memory_version)}\n"
                       f"   Found Version: {str(memory_version)}\n"
                       f"Please follow these steps:\n"
                       f"   Run the OpenGOAL Launcher, click Jak and Daxter > Features > Mods > ArchipelaGOAL.\n"
