...
if TYPE_CHECKING:
    from worlds.oot.Rom import Rom


def _sprite_path(age: int) -> str:
    subfolder = "adult" if age == 0 else "child"
    return user_path("data", "sprites", "oot", subfolder)


def get_model_choices(age: int) -> list[str]:
    names = ["Default"]
    path = _sprite_path(age)
...
def patch_model_adult(rom: Rom, ootworld) -> None:
    # Get model filepath
    model = ootworld.model_adult_filepicker
    # Default to filepicker if non-empty
    if len(model) == 0:
        model = ootworld.model_adult
        if ootworld.model_adult == "Random":
            choices = get_model_choices(0)
            choices.remove("Default")
            choices.remove("Random")
            model = random.choice(choices)
        model = os.path.join(_sprite_path(0), f'{model}.zobj')
...
def patch_model_child(rom: Rom, ootworld) -> None:
    # Get model filepath
    model = ootworld.model_child_filepicker
    # Default to filepicker if non-empty
    if len(model) == 0:
        model = ootworld.model_child
        if ootworld.model_child == "Random":
            choices = get_model_choices(1)
            choices.remove("Default")
            choices.remove("Random")
            model = random.choice(choices)
        model = os.path.join(_sprite_path(1), f'{model}.zobj')
