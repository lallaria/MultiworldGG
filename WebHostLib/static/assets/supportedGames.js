window.addEventListener('load', () => {
  ...
  gameSearch.addEventListener('input', (evt) => {
    ...
    toggleButtons.forEach((header) => {
      ...
      if (header.getAttribute('data-game').toLowerCase().includes(evt.target.value.toLowerCase().trim()) || header.getAttribute('data-display-name').toLowerCase().includes(evt.target.value.toLowerCase().trim())) {
        header.style.display = null;
